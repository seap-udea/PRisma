...
for _p in (str(_REPO / "pipeline"), str(_PIPELINE), str(_REPO)):
    if _p not in sys.path:
        sys.path.append(_p)
sys.path.insert(0, str(_PIPELINE))

# Parse --config early so we can import the correct module top-level
_config_name = "run_sweep"
if "--config" in sys.argv:
    _idx = sys.argv.index("--config")
    if _idx + 1 < len(sys.argv):
        _config_name = sys.argv[_idx + 1]

import importlib
sweep = importlib.import_module(_config_name)
...
def execute_one(case: str, params: dict, n_procs: int, dry_run: bool = False,
                skip_ppc: bool = False, verbose: bool = True, force: bool = False):
...
def main():
    ...
    ap.add_argument("--config", default="run_sweep_full",
                    help="Config module to load (e.g. run_sweep_full, run_sweep_single)")
    ap.add_argument("--model", default=None,
                    help="Override FORWARD_MODEL_VARIANTS (e.g. exorings, geotrans)")
    ...
    if args.model:
        sweep.FORWARD_MODEL_VARIANTS = [m.strip() for m in args.model.split(",")]

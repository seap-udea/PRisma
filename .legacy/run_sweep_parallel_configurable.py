...
import run_sweep_configurable as sweep  # noqa: E402
...
def execute_one(case: str, params: dict, n_procs: int, dry_run: bool = False,
                skip_ppc: bool = False, verbose: bool = True, run_tag_prebuilt: str = None,
                force: bool = False):
    ...
    if not force and (results_dir / f"{run_tag}.npz").exists():
    ...
    try:
        ...
        if ppc is not None:
            arrays["ppc"] = ppc
        saved = pr.save_run(results_dir, run_tag, arrays, meta, overwrite=force)
        if saved is None:
...
def main():
    ...
    payloads = []
    for run in runs:
        # Pass run_label into params so execute_one can use it for build_run_tag
        if "run_label" in run.get("run_tag", ""):
            # wait, actually the run dictionary from build_run_list has the full tag
            # It's better to just pass run_label explicitly if we added it to run dict,
            # or extract it. Let's just modify build_run_list in run_sweep_configurable
            # to also return "run_label" in the run dict.
            pass
            
        params = dict(run["params"])
        # We can extract run_label from the tag if it's there, but run_sweep_configurable 
        # doesn't currently put run_label in the run dict. Let's assume we can parse it from tag,
        # or better: we'll modify the loop below for jobs == 1 to use run["run_tag"] directly,
        # and for jobs > 1, execute_one can just get the exact tag. 
        # Wait, execute_one rebuilds the tag! We must pass run_label in params.
        # Let's extract run_label from the tag if it has one.
        pass 

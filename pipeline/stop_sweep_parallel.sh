...
# =============================================================================
# Sends SIGTERM to the run_sweep_parallel.py process tree. Completed runs are
...
PIDS="$(pgrep -f 'run_sweep_parallel\.py' || true)"

if [[ -z "${PIDS}" ]]; then
    echo "No run_sweep_parallel.py process found."
    exit 0
fi

echo "Found run_sweep_parallel.py PID(s):"
...
LEFT="$(pgrep -f 'run_sweep_parallel\.py' || true)"
if [[ -n "${LEFT}" ]]; then

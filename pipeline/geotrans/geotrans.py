...
from numpy import *
import numpy as np  # Explicit numpy namespace for clarity

# Restore Python built-in functions overwritten by 'from numpy import *'
# This prevents subtle bugs when using max(), min(), etc. with scalars
...
def derivedSystemProperties(S):
    """
    Refactored to work in normalized units (R*=1).
    Primary inputs: rhotrue [g/cm^3], P [days], b [R*], p=Rp/R*.
    No stellar mass/radius or planetary mass/absolute radius required.
    """
    #ROTATION MATRICES
    ...
    #RINGS (all in R* units; p = Rp/R* is a direct input)
    S.Rp=S.p
    S.Ri=S.fi*S.Rp
    S.Re=S.fe*S.Rp

    #STELLAR DENSITY (convert rhotrue from g/cm^3 to kg/m^3)
    S.rho_true=S.rhotrue*1E3  # kg/m^3

    #SEMIMAJOR AXIS a/R* derived from rhotrue and P (Kepler's third law)
    #  a/R* = [ G * rho_true / (3*pi) * P^2 ]^(1/3)
    #  (assumes Mstar >> Mplanet, so Mtotal ~ Mstar)
    S.aRs=(GCONST*S.rho_true/(3*pi)*(S.P*DAY)**2)**(1./3)

    #ORBITAL INCLINATION from impact parameter b = (a/R*) * cos(iorb)
    #  b is already stored in S.b (input); iorb is derived
    cosiorb=S.b/S.aRs
    if abs(cosiorb)>1:
        print("This configuration does not lead to a transit (b > a/R*).")
        exit(1)
    S.iorb=arccos(cosiorb)
    #Update rotation matrices with derived iorb
    S.Mi=rotMat([1,0,0],-S.iorb)
    S.Mos=dot(S.Mi,S.Mw)
    ...
    #CENTRAL ANOMALIES (orbit geometry; eccentric orbit supported)
    S.fcen=270*DEG-S.wp
    S.Ecen=2*arctan(sqrt((1-S.ep)/(1+S.ep))*tan(S.fcen/2))
    S.Mcen=S.Ecen-S.ep*sin(S.Ecen)
    S.tcen=S.Mcen/S.norb

    #CENTRAL RADIUS in R* units (rcen/R*)
    #  For circular orbit: rcen/R* = a/R*
    #  For eccentric orbit: rcen/R* = (a/R*) * (1-e^2)/(1+e*cos(fcen))
    S.rcen=S.aRs*(1-S.ep**2)/(1+S.ep*cos(S.fcen))

    #CENTRAL POSITION AND IMPACT PARAMETER (all in R*)
    rpcen=AR3(S.rcen*cos(S.fcen),S.rcen*sin(S.fcen),0)
    S.Pcen=dot(S.Mos,rpcen)
    S.Borb=S.Pcen[1]   # already in R* units since rcen is in R*

    if abs(S.Borb)>1:
        print("This configuration does not lead to a transit.")
        exit(1)

    #ORBIT OBJECT (dimensionless: a in R*, P in seconds)
    S.orbit=Orbit(S.aRs,S.ep,S.Porb,S.Mos)
...
def updatePlanetRings(S,phir=123,ir=123):
    """
    All quantities in R* units. rcen, aRs are already in R*.
    No Rstar, Rplanet, or ap absolute values needed.
    """
    ...
    # All quantities in R* units: rcen=rcen/R*, aRs=a/R*
    # df here is delta_f (true anomaly change), dimensionless
    # sqrt(1-Borb^2) = sin(iorb) since Borb=(a/R*)*cos(iorb)/rcen_norm
    #//////////////////////////////////////////////////
    siniorb=sqrt(1-S.Borb**2) if abs(S.Borb)<=1 else 0.0
    df=sqrt(1-S.Borb**2)/S.rcen   # delta_f ~ R*/rcen (in R* units, R*=1)
    f15=S.fcen-df
    r15=ellipseRadiusE(S.aRs,S.ep,f15)   # r15 in R* units
    t15=timeOrbit(S.ep,S.norb,f15)
    f35=S.fcen+df
    r35=ellipseRadiusE(S.aRs,S.ep,f35)   # r35 in R* units
    t35=timeOrbit(S.ep,S.norb,f35)
    if t15*t35<0 and t35<0:t35+=S.Porb

    #ESTIMATED TRANSIT TIME
    S.dtrans=t35-t15

    #TIME TO COVER THE PLANETARY RADIUS (Rp/R* = S.Rp, rcen in R*)
    df=S.Rp/S.rcen
    f=S.fcen-df
    t=timeOrbit(S.ep,S.norb,f)
    S.dtplanet=S.tcen-t

    #TIME TO COVER THE RADIUS OF THE STAR (R*/R* = 1, rcen in R*)
    df=1.0/S.rcen
    f=S.fcen-df
    t=timeOrbit(S.ep,S.norb,f)
    S.dtstar=S.tcen-t
...
def systemShow(S):
    pass
    print("Star primary:")
    print(TAB,"Stellar density: rho_true = %.4f g/cm^3"%S.rhotrue)
    print("Planet primary:")
    print(TAB,"Planetary radius: p = Rp/R* = %.6f"%S.p)
    print(TAB,"Oblateness: fp = %.4f"%S.fp)
    print("Rings primary:")
    ...
    print("Orbit primary:")
    print(TAB,"Period: P = %.6f days"%(S.P))
    print(TAB,"Impact parameter: b = %.4f R*"%(S.b))
    print(TAB,"Eccentricity = %.2f"%(S.ep))
    print(TAB,"Periapsis argument = %.2f deg"%(S.wp*RAD))
    print()
    print("Derived orbital properties:")
    print(TAB,"Semimajor axis: a/R* = %.4f"%S.aRs)
    print(TAB,"Orbital Inclination: iorb = %.3f deg"%(S.iorb*RAD))
    ...
    print(TAB,"Mean Angular velocity = %e rad/s"%(S.norb))
    ...
    print(TAB,"Central radius = %e R*"%(S.rcen))
...
def rhoObserved_Kipping(p,tT,tF,P):
    """
    p: Observed planetary radius in star units (delta = (Rp_obs/R*)^2)
    tT: Total transit duration (in hours)
    tF: Duration of full transit (in hours)
    P: Orbital Period (in hours)
    
    Precise formula by Kipping (2014).
    Rstar not needed: result is rho_obs in kg/m^3.
    """
    sfF=(sin(tF*pi/P))**2
    sfT=(sin(tT*pi/P))**2
    Rp=sqrt(p)
...
# All parameters in normalized units (R*=1).
# No stellar mass/radius or planetary mass/absolute radius required.
#########################################
#//////////////////////////////
#STAR
#//////////////////////////////
rhotrue=1.40598,    # Stellar density (g/cm^3)
c1=0.70,            # Limb Darkening coefficient 1
c2=-0.24,           # Limb Darkening coefficient 2
#//////////////////////////////
#ORBIT
#//////////////////////////////
P=365.2446,         # Orbital period (days)
b=0.1875,           # Impact parameter (R*)
ep=0.0,             # Eccentricity
wp=0.0*DEG,         # Argument of periapsis (rad)
#//////////////////////////////
#PLANET (normalized)
#//////////////////////////////
p=0.08,             # Planetary radius Rp/R*
fp=0.0,             # Oblateness
#//////////////////////////////
#RINGS
#//////////////////////////////
fe=RSAT_ARING/RSAT, # Exterior ring radius (Rp units)
fi=RSAT_BRING/RSAT, # Interior ring radius (Rp units)
ir=45.0*DEG,        # Ring inclination (rad)
phir=45.0*DEG,      # Ring roll angle (rad)
tau=1.0,            # Normal opacity
))
...
class RingedSystem(object):
    ...
    def derivedSystemProperties(self):
        ...
        """
        #ROTATION MATRICES
        self.Mi=rotMat([1,0,0],-self.iorb) if hasattr(self,'iorb') else rotMat([1,0,0],0)
        ...
        #STELLAR DENSITY (convert rhotrue from g/cm^3 to kg/m^3)
        self.rho_true=self.rhotrue*1E3  # kg/m^3

        #SEMIMAJOR AXIS a/R* derived from rhotrue and P (Kepler's third law)
        #  a/R* = [ G * rho_true / (3*pi) * P^2 ]^(1/3)
        self.aRs=(GCONST*self.rho_true/(3*pi)*(self.P*DAY)**2)**(1./3)

        #ORBITAL INCLINATION from impact parameter b = (a/R*) * cos(iorb)
        cosiorb=self.b/self.aRs
        if abs(cosiorb)>1:
            print("This configuration does not lead to a transit (b > a/R*).")
            exit(1)
        self.iorb=arccos(cosiorb)
        #Update rotation matrices with derived iorb
        self.Mi=rotMat([1,0,0],-self.iorb)
        self.Mos=dot(self.Mi,self.Mw)

        #ORBITAL PERIOD in seconds
        self.Porb=self.P*DAY
        ...
        #CENTRAL RADIUS in R* units
        self.rcen=self.aRs*(1-self.ep**2)/(1+self.ep*cos(self.fcen))

        #CENTRAL POSITION AND IMPACT PARAMETER (all in R*)
        rpcen=AR3(self.rcen*cos(self.fcen),self.rcen*sin(self.fcen),0)
        self.Pcen=dot(self.Mos,rpcen)
        self.Borb=self.Pcen[1]   # in R* units
        ...
        #ORBIT OBJECT (dimensionless: a in R*, P in seconds)
        self.orbit=Orbit(self.aRs,self.ep,self.Porb,self.Mos)
    ...
    def updatePlanetRings(self):
        ...
        #//////////////////////////////////////////////////
        #ESTIMATED CONTACT TIMES
        # All in R* units: rcen=rcen/R*, aRs=a/R*
        # sqrt(1-Borb^2) is sin(iorb) in normalized coords
        #//////////////////////////////////////////////////
        df=sqrt(1-self.Borb**2)/self.rcen  # delta_f in R* coords
        f15=self.fcen-df
        r15=ellipseRadiusE(self.aRs,self.ep,f15)
        t15=timeOrbit(self.ep,self.norb,f15)
        f35=self.fcen+df
        r35=ellipseRadiusE(self.aRs,self.ep,f35)
        ...
        #TIME TO COVER THE PLANETARY RADIUS (Rp/R* = self.Rp, rcen in R*)
        df=self.Rp/self.rcen
        f=self.fcen-df
        t=timeOrbit(self.ep,self.norb,f)
        self.dtplanet=self.tcen-t

        #TIME TO COVER THE RADIUS OF THE STAR (R*/R* = 1.0, rcen in R*)
        df=1.0/self.rcen
        f=self.fcen-df
        t=timeOrbit(self.ep,self.norb,f)
    ...
    def calculate_rho_obs(self, method='Seager'):
        """
        Calculate the observed stellar density from transit durations.
        Uses the Seager & Mallen-Ornellas (2003) formula.
        No Rstar needed: rho_obs derived purely from transit geometry.
        """
        ...
        if method == 'Kipping':
            rho_obs, a, b =rhoObserved_Kipping(p,
                                self.tT,self.tF,
                                self.Porb/HOUR)
        elif method == 'Seager':
            rho_obs, a, b =rhoObserved_Seager(p,
                                    self.tT,self.tF,
                                    self.Porb/HOUR)
        else:
    ...
    def print(self):
        pass
        print("Star primary:")
        print(TAB,"Stellar density: rho_true = %.4f g/cm^3"%self.rhotrue)
        print("Planet primary:")
        print(TAB,"Planetary radius: p = Rp/R* = %.6f"%self.p)
        print(TAB,"Oblateness: fp = %.4f"%self.fp)
        print("Rings primary:")
        ...
        print("Orbit primary:")
        print(TAB,"Period: P = %.6f days"%(self.P))
        print(TAB,"Impact parameter: b = %.4f R*"%(self.b))
        print(TAB,"Eccentricity = %.2f"%(self.ep))
        print(TAB,"Periapsis argument = %.2f deg"%(self.wp*RAD))
        print()
        print("Derived orbital properties:")
        print(TAB,"Semimajor axis: a/R* = %.4f"%(self.aRs))
        print(TAB,"Orbital Inclination: iorb = %.3f deg"%(self.iorb*RAD))
        print("Planetary derivative:")
        ...
        print(TAB,"Mean Angular velocity = %e rad/s"%(self.norb))
        ...
        print(TAB,"Central radius = %e R*"%(self.rcen))

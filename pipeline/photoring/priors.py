...
pinned to the lower edge of its prior), and ``tau``, ``rho_true``, ``b`` are appended when
...
======================  ==================================================================
 ...
 ``tau`` (opt.)          Log-uniform ``p(tau) ~ 1/tau`` on ``[TAU_LO, TAU_HI]``
...
def build_param_space(model_config):
    ...
    are ``P_FREE`` (default ``True``), ``TAU_FREE``, ``RHO_TRUE_FREE``, ``B_FREE``.
    """
    p_free = bool(model_config.get("P_FREE", True))
    names = ["fe", "ir", "theta"]
    if p_free:
        names.append("p")
    if bool(model_config.get("TAU_FREE", False)):
        names.append("tau")
